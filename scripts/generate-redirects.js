...
function generateRedirects() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const isHighPriority = highPriorityUrls.some(priorityUrl =>
      cleanPath.includes(priorityUrl.replace('/', ''))
    );
    const isSoft404 =
      headers.some(h => h.toLowerCase().includes('error')) &&
    ...
    const redirect = {
      source: cleanPath,
      destination:
        `/new-path/${cleanPath.replace(/\//g, '')}`.replace(/-+/g, '-').replace(/^-|-$/g, '') ||
        '/new-path',
      permanent: permanent,
    };
  ...
  const output = {
    summary: {
      ...
      highPriorityUrls: redirects.filter(
        r => r.permanent && highPriorityUrls.some(url => r.source.includes(url.replace('/', '')))
      ).length,
    },
    redirects: redirects,
  };
